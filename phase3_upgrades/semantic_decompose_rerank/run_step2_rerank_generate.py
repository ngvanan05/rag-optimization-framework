...
OUTPUT_PATH    = ROOT / "outputs" / "semantic_decompose_rerank" / "semantic_decompose_rerank_results.csv"


def main():
    set_global_seed(SEED)
    print("=" * 60)
    print("  COMBO v1+v2+v4 - STEP 2: RERANK + GENERATE")
    print("  v4 Qwen3 Reranker (GPU) + Baseline RAGChain")
    print("=" * 60)

    if not RETRIEVED_PATH.exists():
        print(f"File not found: {RETRIEVED_PATH}")
        print("Please run run_step1_chunk_retrieve.py first.")
        return
