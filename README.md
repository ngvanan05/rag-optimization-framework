...
[Tổng quan](#tổng-quan) · [Kiến trúc](#kiến-trúc) · [Cài đặt](#cài-đặt) · [Hướng dẫn chạy](#hướng-dẫn-chạy) · [Kết quả](#kết-quả-thực-nghiệm) · [EXPERIMENT.vi.md](EXPERIMENT.vi.md)
...
# Cài đặt project ở chế độ editable để dùng import package chuẩn (phase2_baseline.*, ...)
pip install -e .
```
...
> **Reproducibility:** `requirements.txt` dùng version range để linh hoạt cập nhật. Để cài đúng version đã test (pin chính xác), dùng `pip install -r requirements-lock.txt` thay vì `requirements.txt`. File lock được sinh bằng `pip-compile requirements.txt -o requirements-lock.txt` (cần `pip install pip-tools`).
...
> Xem phân tích chi tiết và RAGAS report từng variant: [EXPERIMENT.vi.md](EXPERIMENT.vi.md)
